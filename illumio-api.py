...
def usage():
    ...
    <command>:
    init - setup illumio pce api parameters 
    get - get policy object
    create - create policy object from csv file's
    update - update policy object values


    Most used options for 'init' command:

    -fqdn: Illumio REST API PCE fqdn
    -port: Illumio REST APIs PCE port number
    -version: Illumio REST API release versions( default to 2)
    -org: Illumio REST API   organization HREF
    -username: Illumio REST API username
    -secret: Illumio REST API key secret

    EXAMPLES:
    illumio-api init -fqdn pce.tds.local -port 443 -api 2 -org 1 -username api_183ab0757df31cf75 -secret 0c8b09a6a35a51d0f12fdf960fd2b531970325c1e

    
    Most used options for 'get' command:


    -id: Illumio object id to get


    Most used objets for 'get' command:
    ...
    Most used objets for 'create' command:
    labels
    services
    rules
    ...
    Most used objets for 'update' command:
    labels
    services
    rules
    Most used options for 'update' command:
    -f <csv file name>

    EXAMPLES:

    illumio-api init -fqdn pce.tds.local -port 443 -version 2 -org 1 -username api_183ab0757df31cf75 -secret 0c8b09a6a35a51d0f12fdf960fd2b531970325c1e
    illumio-api update labels -f labels.csv
    illumio-api update services -f services.csv
    illumio-api update rules -f rules.csv
